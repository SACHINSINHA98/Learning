...
public class EquillibriumIndex {
    ...
    private static void equllibrium(List<Integer> list) {
        ...
        for(int j=1;j< prefixSum.size();j++){
             index=j;
             int leftSum=prefixSum.get(j-1);
             int rightSum=prefixSum.get(list.size()-1)-prefixSum.get(j);
             if(leftSum==rightSum){
                 break;
             }
        }
